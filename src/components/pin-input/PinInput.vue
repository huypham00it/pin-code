<template>
  <div class="flex gap-5">
    <input
      ...
      v-bind="$attrs"
      :class="[
        'py-3 px-4 w-12 text-2xl font-bold shadow-md border border-transparent bg-neutral-100 rounded-lg',
        cell.value ? '!border-blue-300' : '',
        inputClass
      ]"
...
import type { Cell, CellInputType, CellsInputTypes } from './PinInput.types'

const BASE_REF_NAME = 'pincode-input'
...
const onPaste = (pasteIdx: number, e: ClipboardEvent) => {
  e.preventDefault()
  const pasteValues: string[] = (e.clipboardData as DataTransfer).getData('text/plain').split('')
